...
class GabrielGraph:
    ...
    def support_vertex_indices(self) -> list[int]:
        """Retorna a lista dos vértices que participam de arestas de suporte."""
        supp = self.support_adjacency()
        mask = torch.any(supp, dim=0)
        return torch.nonzero(mask, as_tuple=False).view(-1).cpu().numpy().tolist()
